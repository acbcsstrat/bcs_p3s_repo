...
public class MoneycorpSftpAccessImpl extends Universal implements MoneycorpSftpAccess {
	...
	@Override
	public void createOrderFile(Order order)  throws P3SPropertyException {  //actmp exception added during quickDev harnees
		...
		try{
			...
			if(lines.size() == 2){
				...
				// Not needed in harness // sftp.upload(lines,writefileName);
			}
			else{
