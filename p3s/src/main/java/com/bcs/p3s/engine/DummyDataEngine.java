...
//        if (pLoginSession==null) logInternalError().fatal("createDummyPatentUiForSearchAddPatent retrieved null session bean");
       ...
       //epo dummy engine
		...
		patent.setFilingDate(nowPlusNdays(-1605));
		//patent.setFilingDate(nowPlusNdays(-505));
		...
		/** If from scrape its been found that no previous renewals being made,
		 * 		then insert values as 0 into those columns 
		 * 
		 */
		patent.setLastRenewedDateExEpo(nowPlusNdays(-570));
