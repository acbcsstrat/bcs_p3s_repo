...
/**
 * Created for v2.1, as such data will (eventually) no longer be calculated for
 * ALL patents on login.
 *
 * Note: This is a *UI object, so RATE values provided will have already been
 * inverted - i.e. WhatEurosDoes1DollarBuy - i.e. = ~0.8
 * 
 * v2.1 - Nov2018 - This is currently the ONLY place in the application where
 * calculations are performed on the WhatEurosDoes1DollarBuy rate rather than
 * the dollarsfor1EuroRate
 * 
 */
public class CostHistoryUI extends Universal {
	...
	public CostHistoryUI(FeeCurrencyComponents fees) {
		...
		setSubTotalEURYesterday(
				(fees.getDollarComponentUSD().multiply(fxRateYesterday)).add(fees.getEuroComponentEUR()));
		setSubTotalUSDYesterday(fees.getDollarComponentUSD()
				.add(fees.getEuroComponentEUR().divide(fxRateYesterday, 2, RoundingMode.HALF_UP)));

		setSubTotalEURLastWeek((fees.getDollarComponentUSD().multiply(fxRateLastWeek)).add(fees.getEuroComponentEUR()));
		setSubTotalUSDLastWeek(fees.getDollarComponentUSD()
				.add(fees.getEuroComponentEUR().divide(fxRateLastWeek, 2, RoundingMode.HALF_UP)));

		setSubTotalEURLastMonth(
				(fees.getDollarComponentUSD().multiply(fxRateLastMonth)).add(fees.getEuroComponentEUR()));
		setSubTotalUSDLastMonth(fees.getDollarComponentUSD()
				.add(fees.getEuroComponentEUR().divide(fxRateLastMonth, 2, RoundingMode.HALF_UP)));
	}
	...
	protected void populateHistoricRates() {
		...
		BigDecimal yesterdayRate = getHistoricRateOrNull(yesterday); // formerly illogically getSafeRate() & protectAgainstMissingDayRates() together
		setFxRateYesterday(CurrencyUtil.invertRateWithDefaults(yesterdayRate, null));
		BigDecimal lastWeekRate = getHistoricRateOrNull(weekAgo);
		setFxRateLastWeek(CurrencyUtil.invertRateWithDefaults(lastWeekRate, yesterdayRate));
		BigDecimal lastMonthRate = getHistoricRateOrNull(monthAgo);
		setFxRateLastMonth(CurrencyUtil.invertRateWithDefaults(lastMonthRate, lastWeekRate));

		//protectAgainstMissingDayRates();
	}

	// protected BigDecimal getSafeRate(Date when) { // Avoid nulls due to missing
	// rate history data (which will occur on DEV PCs)
	// BigDecimal result = getHistoricRateOrNull(when);
	// if (result==null) result =
	// GlobalVariableSole.findOnlyGlobalVariableSole().getCurrent_P3S_rate();
	// return result;
	// }
	/**
	...
	protected BigDecimal getHistoricRateOrNull(Date historicDate) {
		...
		try {
			...
			if (e instanceof EmptyResultDataAccessException) {
				System.out.println(
						"Swallow exception. This would be normal on dev PCs where there is no daily rate feed.");
				/* swallow */
			} else
				throw new P3SRuntimeException("Failed read archivedRate for date " + historicDate, e);
		}
	...
	//	if (isRateEmpty(fxRateYesterday))
	//		fxRateYesterday = GlobalVariableSole.findOnlyGlobalVariableSole().getCurrent_P3S_rate();
	//	if (isRateEmpty(fxRateLastWeek))
	//		fxRateLastWeek = fxRateYesterday;
	//	if (isRateEmpty(fxRateLastMonth))
	//		fxRateLastMonth = fxRateLastWeek;
	//}

	protected boolean isRateEmpty(BigDecimal aRate) {
		if (aRate == null || aRate.compareTo(BigDecimal.ZERO) <= 0)
			return true;
		else
			return false;
	}
