...
/**
 * Provides odd functionality. Usually for Internal Use Only, & provided from
 * the actest.html page
 * 
 * @author andyc
 *
 */
...
public class MiscController extends Universal implements Form1200HarnessConstants {
	...
	public String listtxns(Model uiModel) {
		...
		for (Payment pay : allPayments) {
			...
			if ("Completed".equalsIgnoreCase(pay.getLatestTransStatus()))
				continue;
			Invoice ii = pay.getLatestInvoice();
			if (ii != null && ("Closed".equals(ii.getInvoiceStatus())))
				continue;
			;
	...
	public String listtxnproducts(Model uiModel, String p3sref) {
		...
		for (Renewal renewal : renewals) {
			...
			TxnProduct product = new TxnProduct("Renewal", renewal.getId(), patent.getEP_ApplicationNumber(),
					patent.getId(), renewal.getRenewalStatus(), null, -1L);
		...
		for (Epct epct : epcts) {
			...
			TxnProduct product = new TxnProduct("Euro-PCT", epct.getId(), patent.getEP_ApplicationNumber(),
					patent.getId(), epct.getEpctStatus(), epctoptions, blobId);
			products.add(product);
		}
	...
	// produces = "text/html")
	// public String changeproductstatus(Model uiModel, String productType, String
	// productId, String nextstate) {
	// String loc = "MiscController :: changeproductstatus() :
	// productType="+productType+" productId="+productId+" and new status is
	// "+nextstate+" : ";
	// log().debug(loc+"invoked ");;
	//
	...
	// }
	// else log().error("Change ionvoked, yet not appropriate for
	// productId="+productId+" : from "+loc);
	// //uiModel.addAttribute("TxnRef", txnName); // would love to - but need
	// Payment
	//
	// return "done";
	// }
		...
		try {
			...
			if (blobby == null) {
				...
				while ((size = sImage.read(bytearray)) != -1) {
					log().debug(err + " in loop reading f1200 pdf. Size this loop iteration is " + size);
					response.setContentType("application/pdf");
			...
			logInternalError().error(err + "Error retrieving f1200 pdf. Rqst was Epct id " + epctId+", "+isDownloadNotView, ex);
		}
	}
